...
const main = async () => {
    ...
    // Step 2: Get global state root hash against which to issue queries.
    const stateRootHash = await getStateRootHash(client);

    // Step 3: Get hash of installed contract - should be cached.
    const contractHash = await getContractHash(client, stateRootHash, keyPair);

    // Step 4: Get on-chain token information.
    console.log({
...
const getContractKeyValue = async (client, stateRootHash, stateKey, statePath) => {
    ...
    const { CLValue: { parsed: value } } = await client.nodeClient.getBlockState(
        stateRootHash,
        stateKey,
        [statePath]
    );
